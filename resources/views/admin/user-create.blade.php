			...
			<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-4" data-widget-editbutton="false" data-widget-custombutton="false">
				...
				<div>
					...
					<div class="widget-body no-padding">
						...
						<form class="smart-form" method="post" action="@if(isset($info)) {{ url('admin/user',$info['id']) }} @else {{ url('admin/user') }} @endif">
							@if(isset($info))
							{{ method_field('put') }}
							@endif
							<header>
								管理员@if (isset($info)) 修改 @else 添加 @endif
							</header>
							...
							<fieldset>
								...
								<section>
									...
									<label class="input"> <i class="icon-prepend fa fa-question-circle"></i>
										<input type="text" disabled="disabled" name="name" placeholder="" value="@if(isset($info)){{ $info['name'] }}@else{{ old('name') }}@endif" required="required">
