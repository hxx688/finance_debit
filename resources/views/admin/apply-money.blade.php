			...
			<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-4" data-widget-editbutton="false" data-widget-custombutton="false">
				...
				<div>
					...
					<div class="widget-body no-padding">
						<form class="smart-form" method="post" action="@if(isset($info)) {{ url('admin/applysOpt',$info['id']) }} @else {{ url('admin/applysOpt') }} @endif">
							@if(isset($info))
							{{ method_field('put') }}
							@endif
							<header>
								@if (isset($info)) 修改 @else 添加 @endif
							</header>
							...
							<fieldset>
								...
								<section>
									...
									<label class="input"> <i class="icon-prepend fa fa-question-circle"></i>
										<input type="text" name="money" placeholder="" value="@if(isset($info)){{ $info['money'] }}@else{{ old('money') }}@endif" required="required">
									</label>
								</section>
							</fieldset>
							{{ csrf_field() }}
